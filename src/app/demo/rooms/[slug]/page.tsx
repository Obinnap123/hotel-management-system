...
export default async function PublicRoomDetailsPage({ params }: Props) {
  const roomType = await getPublicRoomTypeBySlug((await params).slug);
  if (!roomType) notFound();
  const available = roomType.availableRoomCount > 0;
      ...
      <div className="grid gap-10 lg:grid-cols-[1.08fr_.92fr] lg:items-start lg:gap-16">
        ...
        <div className="lg:sticky lg:top-28">
          ...
          <dl className="mt-8 grid grid-cols-2 border-y border-[#d9d3c8] sm:grid-cols-3">
            <RoomFact icon={<BedDouble />} label="Nightly rate" value={roomType.pricePerNight ? formatPublicCurrency(roomType.pricePerNight) : "On request"} />
            <RoomFact icon={<Users />} label="Sleeps" value={`${roomType.capacity ?? "—"} guests`} />
            <div className="col-span-2 border-t border-[#d9d3c8] px-4 py-5 sm:col-span-1 sm:border-l sm:border-t-0"><dt className="text-[0.62rem] font-bold uppercase tracking-[0.14em] text-[#7c7164]">Availability</dt><dd className={`mt-2 text-sm font-semibold ${available ? "text-[#28705b]" : "text-[#8a5142]"}`}>{available ? `${roomType.availableRoomCount} ready to reserve` : "Please ask reception"}</dd></div>
          </dl>
