  ...
  return (
    <form action={formAction} className="mt-10 space-y-8">
      ...
      <fieldset className="space-y-5">
        <legend className="mb-5 font-serif text-2xl text-[#22312d]">Your details</legend>
      ...
      <fieldset className="space-y-5 border-t border-[#d9d3c8] pt-8">
        <legend className="pr-4 font-serif text-2xl text-[#22312d]">Stay details</legend>
        <div className="grid gap-5 sm:grid-cols-2">
          <Field className="sm:col-span-2" error={state.fieldErrors?.roomTypeSlug?.[0]} id={`${prefix}-room`} label="Room type"><select aria-describedby={state.fieldErrors?.roomTypeSlug ? `${prefix}-room-error` : undefined} aria-invalid={Boolean(state.fieldErrors?.roomTypeSlug)} className={controlClass} defaultValue={defaultRoomTypeSlug ?? ""} id={`${prefix}-room`} name="roomTypeSlug" required><option value="">Choose a room type</option>{roomTypes.map((roomType) => <option disabled={roomType.availableRoomCount === 0} key={roomType.slug} value={roomType.slug}>{roomType.name}{roomType.availableRoomCount === 0 ? " — unavailable" : ""}</option>)}</select></Field>
      ...
      <button className="inline-flex h-13 w-full items-center justify-center gap-3 bg-[#173b32] px-6 text-xs font-bold uppercase tracking-[0.14em] text-white transition hover:bg-[#225044] disabled:opacity-65" disabled={pending || roomTypes.length === 0} type="submit">{pending ? <><LoaderCircle aria-hidden="true" className="h-4 w-4 animate-spin" /> Reserving your stay</> : <>Complete reservation <ArrowRight aria-hidden="true" className="h-4 w-4" /></>}</button>
      <p className="text-center text-xs leading-5 text-[#737c77]">Your reservation is sent directly to hotel reception. Payment and final room assignment are handled by hotel staff.</p>
    </form>
  );
}
...
function Field({ children, className = "", error, hint, id, label }: { children: React.ReactNode; className?: string; error?: string; hint?: string; id: string; label: string }) {
  ...
  return <div className={className}><div className="flex items-baseline justify-between gap-3"><label className="text-xs font-bold uppercase tracking-[0.12em] text-[#4f5d58]" htmlFor={id}>{label}</label>{hint ? <span className="text-xs text-[#858b87]">{hint}</span> : null}</div>{children}{error ? <p className="mt-2 text-sm text-red-700" id={errorId}>{error}</p> : null}</div>;
}

const controlClass = "mt-2 h-12 w-full border border-[#d3ccc0] bg-white px-3 text-sm outline-none transition focus:border-[#a67c45]";
