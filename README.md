...
Set `NEXT_PUBLIC_DEPLOYMENT_MODE` separately in each Vercel project:

- `marketing` keeps the SymplyUp website at `/` and the reservation preview at `/demo`.
- `hotel` serves the reservation website at `/`, with clean `/rooms` and `/book` URLs while retaining `/login` and `/dashboard`.
